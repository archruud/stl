...
# Dropdown Terminal
# MODIFISERT: Lagt til støtte for ghostty terminal
...
terminal_exists() {
    ...
    if [ -n "$addr" ]; then
        # Get window info
        local window_info=$(hyprctl clients -j | jq -r --arg ADDR "$addr" \
            '.[] | select(.address == $ADDR) | "\(.class)|\(.floating)|\(.workspace.name)"')

        if [ -z "$window_info" ]; then
            debug_echo "Terminal validation: FAILED - no window found at $addr, clearing cache"
            rm -f "$ADDR_FILE"
            return 1
        fi

        local class=$(echo "$window_info" | cut -d'|' -f1)
        local is_floating=$(echo "$window_info" | cut -d'|' -f2)
        local workspace=$(echo "$window_info" | cut -d'|' -f3)

        debug_echo "Terminal validation: class=$class, floating=$is_floating, workspace=$workspace"

        # ═══════════════════════════════════════════════════════════════
        # MODIFISERT: Lagt til støtte for ghostty (com.mitchellh.ghostty)
        # ═══════════════════════════════════════════════════════════════
        # Check 1: Must be a terminal (inkluderer nå ghostty)
        if [[ "$class" != "kitty" && "$class" != "foot" && "$class" != "alacritty" && "$class" != "com.mitchellh.ghostty" ]]; then
            debug_echo "Terminal validation: FAILED - not a terminal class ($class), clearing cache"
            rm -f "$ADDR_FILE"
            return 1
        fi

        # Check 2: Must be floating OR in scratchpad (dropdown is always floating)
        if [[ "$is_floating" != "true" && "$workspace" != "special:scratchpad" ]]; then
            debug_echo "Terminal validation: FAILED - not floating and not in scratchpad (tiled terminal), clearing cache"
            rm -f "$ADDR_FILE"
            return 1
        fi

        debug_echo "Terminal validation: SUCCESS - valid dropdown terminal at $addr"
        return 0
    else
        debug_echo "Terminal validation: No stored address found"
        return 1
    fi
}
